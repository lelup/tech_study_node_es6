...
const ADDRESS_SCHEMA = "Address" // định nghĩa thêm một bảng nửa, bảng address
...
const AddressSchema = {
    ...
    properties: {
        ...
        street: 'string',        
        city: 'string',        
        state: 'string?',//optional property, không bắt buộc        
    }
}
...
const UserSchema = {
    ...
    properties: {
        ...
        email: 'string',  
        addresses: { type: 'list', objectType:  ADDRESS_SCHEMA}, // là một mảng, để thể hiện thuộc tính 1 -> nhiều            
    }
}
const databaseOptions = {
    path: 'RealmInNodeJS.realm',
    schema: [UserSchema, AddressSchema], // list hết danh sách các schema 
...
const insertAddressToUser = (userId, addressObject) => new Promise((resolve, reject) => {                
    Realm.open(databaseOptions).then(realm => {        
        ...
        let filteredAddresses = userObject.addresses
                                ...
                                // filtered: kiểm tra xem addresses truyền vào có street và city trùng ko      
        ...
        realm.write(() => {
            ...
            userObject.addresses.push(newAddress) // giống như thêm 1 object vào mảng
            resolve()
        })
    }).catch((error) => reject(error))
})
const deleteUser = (userId) => new Promise((resolve, reject) => {
    Realm.open(databaseOptions).then(realm => {
        let userObject = realm.objectForPrimaryKey(USER_SCHEMA, userId);       
        // kiểm tra xem userId có tồn tại chưa         
        if (!userObject) {
            reject(`Cannot find user with ID=${userId} to delete`)            
        }      
        realm.write(() => {
            realm.delete(userObject.addresses); // xóa address trước
    ...
    Realm.open(databaseOptions).then(realm => {
        realm.write(() => {
            ...
            for (var index in allUsers) {
                ...
                // xóa từng mảng addresses của từng user
            }
            realm.delete(allUsers)
            // sau đó xóa mảng user
            resolve()
        });
    }).catch((error) => reject(error))
})
