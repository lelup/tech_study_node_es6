...
const filterUsersByName = (searchedName) => new Promise((resolve, reject) => {
    Realm.open(databaseOptions).then(realm => {
        ...
        // trường name có kí tự searchNAme, [c]: ko phân biệt hoa thường
...
const updateAnUser = (userId, updatingUser) => new Promise((resolve, reject) => {
    Realm.open(databaseOptions).then(realm => {
        realm.write(() => {
            let user = realm.objectForPrimaryKey(USER_SCHEMA, userId) // hàm lấy ra User có userId như trên
            // kiểm tra xem user có tồn tại không mới updated
            if (!user) {
            ...
            // updated giá trị mới cho name, email
            resolve()
        });
    }).catch((error) => reject(error))
})
