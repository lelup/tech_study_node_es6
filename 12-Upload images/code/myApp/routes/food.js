...
router.put('/update_a_food', (request, response, next) => {
    ...
    if (request.body.image_name && request.body.image_name.length > 0) {
        ...
        const serverName = require("os").hostname(); // os: thư viện operating systems
...
router.post('/upload_images', (request, response, next) => {
    let formidable = require('formidable'); //formidable: thư viện quản lý việc upload 1 nhiều ảnh trên server
    // parse a file upload
    var form = new formidable.IncomingForm();
    form.uploadDir = "./uploads"; // thư mục upload
    form.keepExtensions = true; // tên file giữ nguyên cả đuôi
    form.maxFieldsSize = 10 * 1024 * 1024; //10 MB - dung lượng 1 file tối đa
    form.multiples = true; // cho phép upload nhiều ảnh
    form.parse(request, (err, fields, files) => {
        ...
        if(files[""] instanceof Array) {
            arrayOfFiles = files[""];
            // lấy ra mảng các file, 1 object có key để trống
        } else {
            arrayOfFiles.push(files[""]);
        }
        
        if (arrayOfFiles.length > 0) {
            var fileNames = []; // mảng chứa các tên file: abc.jpg
            arrayOfFiles.forEach((eachFile)=> {
                // fileNames.push(eachFile.path)
                // duyệt từng mảng, đẩy dần đường dẫn vào mảng fileNames
...
router.get('/open_image', (request, response, next) => {
    let imageName = "uploads/" + request.query.image_name; //imageName: đường dẫn chi tiết đến file trên server
    fs.readFile(imageName, (err, imageData) => {
        ...
        response.writeHead(200, {'Content-Type': 'image/jpeg'}); // kiểu ảnh
