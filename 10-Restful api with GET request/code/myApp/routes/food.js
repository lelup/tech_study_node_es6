    ...
    Food.find({}).limit(100).sort({name: 1}).select({
        ...
        if (err) {
            ...
            response.json({
                result: "ok",
                data: foods,
                count: foods.length, // tính tổng số bàn ghi trong bảng
...
router.get('/get_food_with_id', (request, response, next) => {
    Food.findById(require('mongoose').Types.ObjectId(request.query.food_id), //Types.ObjectId chuyển string -> objectId
        (err, food) => {
            if (err) {
                response.json({
                    result: "failed",
...
router.get('/list_foods_with_criteria', (request, response, next) => {
    ...
    let criteria = {
        ...
        // tên yêu cầu không phân biệt hoa thường, chỉ cần chứa abc là được
    };
